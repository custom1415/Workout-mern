...
//       } transition-all   flex items-center w-full md:max-w-sm p-4 mb-4 text-gray-600 bg-white md:rounded-lg shadow  absolute  md:right-4 md:top-4 top-0`}
//       role="alert"
//     >
//       {success ? (
...
export default function Toast({ visible, message, success }) {
  ...
  return (
    <div
      ...
      className={`${
        visible
          ? "translate-y-0 opacity-100 z-[10000]"
          : "-translate-y-2 opacity-0 z-0"
      } transition-all   flex items-center w-full md:max-w-sm p-4 mb-4 text-gray-600 bg-white md:rounded-lg shadow  absolute  md:right-4 md:top-4 top-0`}
      role="alert"
    >
